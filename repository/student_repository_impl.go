...
func (r *StudentRepositoryImpl) FindAll(ctx context.Context, tx *sql.Tx) (*[]domain.Student, error) {
	...
	for rows.Next() {
		...
		if rows.Scan(&student.ID, &student.Name, &student.Gender, &student.IdentityNumber, &student.Major, &student.Class, &student.Religion, &student.CreatedAt, &student.UpdatedAt); err != nil {
			return nil, err
		}
